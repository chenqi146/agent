...
@logger()
class RedisClient:
    def __init__(self, host: str, port: int, password, database: int):
        # 空密码时不传 password 参数；密码统一转为 str，与 Redis AUTH 协议一致
        pwd = (password if isinstance(password, str) else str(password)).strip() if password else ""
        if pwd:
            self.redis_client = redis.Redis(host=host, port=port, password=pwd, db=database)
        else:
            self.redis_client = redis.Redis(host=host, port=port, db=database)
...
class RedisStackClient:
    ...
    def set_with_expire(self, key: str, value: Any, expire_time: int):
        return self.redis_client.set_with_expire(key, value, expire_time)
    ...
    def get_all_keys(self):
        return self.redis_client.get_all_keys()

    def publish(self, channel: str, message: Any):
        return self.redis_client.publish(channel, message)

    def subscribe(self, *channels):
        return self.redis_client.subscribe(*channels)

    def subscribe_with_callback(self, channel: str, callback: Callable[[dict], None]):
        return self.redis_client.subscribe_with_callback(channel, callback)


class RedisTemplete:
    redis_client: RedisStackClient = None
    lock = threading.Lock()
    is_init = False
    ...
    def init(config: Dict[str, Any]):
        ...
        db = int(config.get('database', config.get('db', 0)))
        pw = config.get('password')
        pw_str = (pw if isinstance(pw, str) else (str(pw) if pw is not None else '')).strip() if pw else ''

        def _connect(use_password: str):
            client = RedisClient(host, port, use_password, db)
            return RedisStackClient(client)

        def _ping():
            RedisTemplete.redis_client.ping()

        try:
            RedisTemplete.redis_client = _connect(pw_str)
            try:
                _ping()
            except Exception as e:
                err_msg = str(e).strip()
                if pw_str and ("without any password configured" in err_msg or "no password is set" in err_msg.lower()):
                    RedisTemplete.redis_client.close()
                    RedisTemplete.redis_client = _connect("")
                    try:
                        _ping()
                        print("RedisTemplete init success (无密码连接): host=%s port=%s db=%s — 当前 Redis 未配置 requirepass，已使用无密码连接" % (host, port, db))
                        ...
                        print("RedisTemplete init ping failed: host=%s port=%s db=%s — %s" % (host, port, db, e2))
                        return False
                RedisTemplete.redis_client.close()
                RedisTemplete.redis_client = None
                RedisTemplete.is_init = False
                print("RedisTemplete init ping failed: host=%s port=%s db=%s — %s (请确认 Redis 已启动: redis-server 或 systemctl start redis)" % (host, port, db, e))
                return False
            RedisTemplete.is_init = True
            print("RedisTemplete init success: host=%s port=%s db=%s" % (host, port, db))
            return True
        except Exception as e:
            print("RedisTemplete init error: %s (请确认 Redis 已启动且 host/port 正确)" % e)
    ...
    def deinit():
        if RedisTemplete.redis_client is not None:
            RedisTemplete.redis_client.close()
            print("RedisTemplete deinit success")
            RedisTemplete.redis_client = None
            RedisTemplete.is_init = False
    ...
    def test():
        ...
        try:
            if RedisTemplete.is_init and RedisTemplete.redis_client is not None:
                ...
                print("RedisTemplete test skipped: not init or redis_client is None")
                return False
        except Exception as e:
    ...
    def set(key: str, value: Any, expire_time: int = 0):
        ...
        try:
            if RedisTemplete.is_init and RedisTemplete.redis_client is not None:
                ...
                print("RedisTemplete set skipped: not init or redis_client is None")
        finally:
            RedisTemplete.lock.release()
    ...
    def get(key: str):
        ...
        try:
            if RedisTemplete.is_init and RedisTemplete.redis_client is not None:
                ...
                print("RedisTemplete get skipped: not init or redis_client is None")
                return None
        finally:
            RedisTemplete.lock.release()
    ...
    def delete(key: str):
        ...
        try:
            if RedisTemplete.is_init and RedisTemplete.redis_client is not None:
                ...
                print("RedisTemplete delete skipped: not init or redis_client is None")
        finally:
            RedisTemplete.lock.release()
    ...
    def exists(key: str):
        ...
        try:
            if RedisTemplete.is_init and RedisTemplete.redis_client is not None:
                ...
                print("RedisTemplete exists skipped: not init or redis_client is None")
                return False
        finally:
            RedisTemplete.lock.release()
