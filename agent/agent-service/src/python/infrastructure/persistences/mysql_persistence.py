...
class MysqlPersistence:
    ...
    def connect(self) -> ErrorCode:
        """
        Connect to database (get connection from pool)
        """
        try:
            if self.connection:
                try:
            ...
            # self.log.info(f"Successfully obtained connection from pool: {self.host}:{self.port}/{self.database}")
            return ErrorCode.SUCCESS
        except Exception as e:
            self.log.error(f"Failed to get connection from pool: {self.host}:{self.port}/{self.database}, error: {str(e)}")
            return ErrorCode.DATABASE_CONNECTION_ERROR
            
    def close(self):
        """
        Close database connection (return to pool)
        """
        try:
            if self.connection:
            ...
            self.log.error(f"Failed to close connection: {str(e)}")
    ...
    def execute_non_query(self, sql: str, params: Optional[Tuple] = None) -> Tuple[ErrorCode, int]:
        """
        Execute non-query SQL statement (INSERT, UPDATE, DELETE) and return affected rows
        ...
        try:
            ...
            self.log.error(f"Failed to execute non-query SQL: {sql}, params: {params}, error: {str(e)}")
    ...
    def begin_transaction(self) -> ErrorCode:
        ...
        try:
            ...
            self.log.error(f"Failed to begin transaction: {e}")
    ...
    def commit(self) -> ErrorCode:
        ...
        try:
            ...
            self.log.error(f"Failed to commit transaction: {e}")
            return ErrorCode.DATABASE_TRANSACTION_ERROR
    ...
    def rollback(self) -> ErrorCode:
        ...
        try:
            ...
            self.log.error(f"Failed to rollback transaction: {e}")
            return ErrorCode.DATABASE_TRANSACTION_ERROR
